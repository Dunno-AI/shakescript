...
class AIService:
    ...
    def extract_metadata(self, user_prompt: str) -> Dict:
        ...
        - Title: What could be a good title for this story?
        - Settings: Where does the story take place? Important places.
        - Special Instructions: Any specific details mentioned by the user which you have to keep in mind while generating the story?
        - Dont give main characters and supporting characters separate.
        ...
        IMPORTANT: Return ONLY valid JSON format with no additional text or explanations.
    ...
    def generate_episode_helper(
        ...
        settings_data = "\n".join(f"Place: {s['Place']}, Description: {s['Description']}" for s in settings)
        ...
        if episode_number == 1:
            ...
            prev_episodes_text = "\n\n".join(
                f"EPISODE {ep_num} SUMMARY: {summary}" for ep_num, _, summary in prev_episodes
            ) if prev_episodes else ""

        phase = "INTRODUCTION" if "Intro" in episode_info else "BUILDING ACTION" if "Building" in episode_info else "CLIMAX" if "Climax" in episode_info else "CONCLUSION"
